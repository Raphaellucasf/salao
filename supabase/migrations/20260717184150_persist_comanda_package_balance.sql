-- Persiste o saldo de pacote usado em cada item para edicoes seguras.
alter table public.comanda_itens add column if not exists pacote_cliente_id uuid references public.pacotes_cliente(id) on delete set null;
CREATE OR REPLACE FUNCTION public.save_comanda_atomic(p_comanda_id bigint, p_client_id bigint, p_professional_id uuid, p_auxiliary_id uuid, p_schedule_date date, p_start_time time without time zone, p_notes text, p_items jsonb, p_admin_id uuid, p_unit_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
  ...
  loop
    ...
    if v_item_type='produto' then
      ...
      insert into public.comanda_itens(comanda_id,tipo,item_id,descricao,quantidade,valor_unitario,
        valor_total,profissional_id,pacote_cliente_id)
      values(v_command.id,'servico',v_item_id::text,v_service.nome,v_quantity,v_value,
        round(v_value*v_quantity,2),p_professional_id,
        nullif(v_item->>'pacote_cliente_id','')::uuid) returning id into v_command_item_id;
